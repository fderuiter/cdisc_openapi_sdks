...
class CtCodelist {
  /// Returns a new [CtCodelist] instance.
  CtCodelist({
    this.conceptId,
    this.extensible,
    this.name,
  ...
  List<CtPackageTerm> terms;

  @override
  bool operator ==(Object other) => identical(this, other) || other is CtCodelist &&
  ...
  @override
  String toString() => 'CtCodelist[conceptId=$conceptId, extensible=$extensible, name=$name, submissionValue=$submissionValue, definition=$definition, preferredTerm=$preferredTerm, synonyms=$synonyms, links=$links, terms=$terms]';
  ...
  /// Returns a new [CtCodelist] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CtCodelist? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CtCodelist[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CtCodelist[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return CtCodelist(
        ...
        synonyms: json[r'synonyms'] is Iterable
            ...
            : const [],
        links: CtCodelistLinks.fromJson(json[r'_links']),
        terms: CtPackageTerm.listFromJson(json[r'terms']),
      );
    }
    return null;
  }

  static List<CtCodelist> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CtCodelist>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CtCodelist.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CtCodelist> mapFromJson(dynamic json) {
    final map = <String, CtCodelist>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CtCodelist.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CtCodelist-objects as value to a dart map
  static Map<String, List<CtCodelist>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CtCodelist>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CtCodelist.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
