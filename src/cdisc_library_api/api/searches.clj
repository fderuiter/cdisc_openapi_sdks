(ns cdisc-library-api.api.searches
...
(defn-spec mdr-search-get-with-http-info any?
  "
  Get Search Results Across CDISC Library"
  ([q string?, ] (mdr-search-get-with-http-info q nil))
  ([q string?, {:keys [highlight start pageSize class codelist conceptId core dataStructure datasetStructure definition description domain effectiveDate extensible href label measureType name preferredTerm product productGroup registrationStatus roleDescription sdtmTarget simpleDatatype submissionValue synonyms type uiHref valueDomain variableSet version]} (s/map-of keyword? any?)]
   (check-required-params q)
   (call-api "/mdr/search" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"q" q "highlight" highlight "start" start "pageSize" pageSize "class" class "codelist" codelist "conceptId" conceptId "core" core "dataStructure" dataStructure "datasetStructure" datasetStructure "definition" definition "description" description "domain" domain "effectiveDate" effectiveDate "extensible" extensible "href" href "label" label "measureType" measureType "name" name "preferredTerm" preferredTerm "product" product "productGroup" productGroup "registrationStatus" registrationStatus "roleDescription" roleDescription "sdtmTarget" sdtmTarget "simpleDatatype" simpleDatatype "submissionValue" submissionValue "synonyms" synonyms "type" type "uiHref" uiHref "valueDomain" valueDomain "variableSet" variableSet "version" version }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["basicAuth"]})))

(defn-spec mdr-search-get default-search-response-spec
  "
  Get Search Results Across CDISC Library"
  ([q string?, ] (mdr-search-get q nil))
  ([q string?, optional-params any?]
   (let [res (:data (mdr-search-get-with-http-info q optional-params))]
     (if (:decode-models *api-context*)
        (st/decode default-search-response-spec res st/string-transformer)
        res))))


(defn-spec mdr-search-scopes-get-with-http-info any?
  "
  Get Search Results Across CDISC Library"
  []
  (call-api "/mdr/search/scopes" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec mdr-search-scopes-get -mdr-search-scopes-get-200-response-spec
  "
  Get Search Results Across CDISC Library"
  []
  (let [res (:data (mdr-search-scopes-get-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode -mdr-search-scopes-get-200-response-spec res st/string-transformer)
       res)))


(defn-spec mdr-search-scopes-scope-get-with-http-info any?
  "
  Get Search Results Limited to Scope"
  [scope default-search-scopes]
  (check-required-params scope)
  (call-api "/mdr/search/scopes/{scope}" :get
            {:path-params   {"scope" scope }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec mdr-search-scopes-scope-get scope-values-spec
  "
  Get Search Results Limited to Scope"
  [scope default-search-scopes]
  (let [res (:data (mdr-search-scopes-scope-get-with-http-info scope))]
    (if (:decode-models *api-context*)
       (st/decode scope-values-spec res st/string-transformer)
       res)))
