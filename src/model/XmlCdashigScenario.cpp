...
void XmlCdashigScenario::validate()
{
    // TODO: implement validation
}

web::json::value XmlCdashigScenario::toJson() const
{
...
bool XmlCdashigScenario::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("self"))))
    {
        ...
        {
            std::shared_ptr<CdashigScenario> refVal_setSelf;
...
void XmlCdashigScenario::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
bool XmlCdashigScenario::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
...
std::shared_ptr<CdashigScenario> XmlCdashigScenario::getSelf() const
{
    return m_Self;
}


void XmlCdashigScenario::setSelf(const std::shared_ptr<CdashigScenario>& value)
{
    m_Self = value;
    m_SelfIsSet = true;
}

bool XmlCdashigScenario::selfIsSet() const
{
    return m_SelfIsSet;
}

void XmlCdashigScenario::unsetSelf()
{
    m_SelfIsSet = false;
}
