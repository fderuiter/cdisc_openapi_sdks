...
case class SendigProduct(
    ...
    links: Option[SendigProductLinks] = None,
    classes: Option[Seq[SendigClass]] = None
)
  
object SendigProduct {
  given encoderSendigProduct: Encoder[SendigProduct] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderSendigProduct: Decoder[SendigProduct] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[SendigProductLinks]]
      classes <- c.downField("classes").as[Option[Seq[SendigClass]]]
    } yield SendigProduct(
      name = name,
      label = label,
