...
case class AdamVariable(
    ...
    links: Option[AdamVariableLinks] = None
)
  
object AdamVariable {
  given encoderAdamVariable: Encoder[AdamVariable] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderAdamVariable: Decoder[AdamVariable] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[AdamVariableLinks]]
    } yield AdamVariable(
      ordinal = ordinal,
      name = name,
      label = label,
