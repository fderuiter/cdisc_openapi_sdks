...
case class CtPackage(
    ...
    links: Option[CtPackageLinks] = None,
    codelists: Option[Seq[CtPackageCodelists]] = None
)
  
object CtPackage {
  given encoderCtPackage: Encoder[CtPackage] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderCtPackage: Decoder[CtPackage] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[CtPackageLinks]]
      codelists <- c.downField("codelists").as[Option[Seq[CtPackageCodelists]]]
    } yield CtPackage(
      name = name,
      label = label,
