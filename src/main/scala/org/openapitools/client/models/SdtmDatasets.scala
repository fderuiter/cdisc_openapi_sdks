...
case class SdtmDatasets(
    ...
    links: Option[SdtmDatasetsLinks] = None
)
  
object SdtmDatasets {
  given encoderSdtmDatasets: Encoder[SdtmDatasets] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderSdtmDatasets: Decoder[SdtmDatasets] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[SdtmDatasetsLinks]]
    } yield SdtmDatasets(
      name = name,
      label = label,
