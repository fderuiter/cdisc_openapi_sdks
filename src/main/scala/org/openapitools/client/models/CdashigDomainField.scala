...
case class CdashigDomainField(
    ...
    links: Option[CdashigDomainFieldLinks] = None
)
  
object CdashigDomainField {
  given encoderCdashigDomainField: Encoder[CdashigDomainField] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderCdashigDomainField: Decoder[CdashigDomainField] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[CdashigDomainFieldLinks]]
    } yield CdashigDomainField(
      ordinal = ordinal,
      name = name,
      label = label,
