...
case class CdashProductDomains(
    ...
    links: Option[CdashProductDomainsLinks] = None
)
  
object CdashProductDomains {
  given encoderCdashProductDomains: Encoder[CdashProductDomains] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderCdashProductDomains: Decoder[CdashProductDomains] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[CdashProductDomainsLinks]]
    } yield CdashProductDomains(
      name = name,
      label = label,
