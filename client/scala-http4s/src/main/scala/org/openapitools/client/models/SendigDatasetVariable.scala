...
object SendigDatasetVariable {
  given encoderSendigDatasetVariable: Encoder[SendigDatasetVariable] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderSendigDatasetVariable: Decoder[SendigDatasetVariable] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[SendigDatasetVariableLinks]]
    } yield SendigDatasetVariable(
      ordinal = ordinal,
      name = name,
      label = label,
