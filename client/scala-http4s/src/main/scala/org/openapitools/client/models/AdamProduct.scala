...
case class AdamProduct(
    ...
    links: Option[AdamProductLinks] = None,
    dataStructures: Option[Seq[AdamDatastructure]] = None
)
  
object AdamProduct {
  given encoderAdamProduct: Encoder[AdamProduct] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderAdamProduct: Decoder[AdamProduct] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[AdamProductLinks]]
      dataStructures <- c.downField("dataStructures").as[Option[Seq[AdamDatastructure]]]
    } yield AdamProduct(
      name = name,
      label = label,
