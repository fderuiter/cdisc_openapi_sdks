...
case class QrsItems(
    ...
    links: Option[QrsItemsLinks] = None
)
  
object QrsItems {
  given encoderQrsItems: Encoder[QrsItems] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderQrsItems: Decoder[QrsItems] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[QrsItemsLinks]]
    } yield QrsItems(
      name = name,
      label = label,
