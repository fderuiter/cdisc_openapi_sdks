...
case class SendigDatasets(
    ...
    links: Option[SendigDatasetsLinks] = None
)
  
object SendigDatasets {
  given encoderSendigDatasets: Encoder[SendigDatasets] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderSendigDatasets: Decoder[SendigDatasets] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[SendigDatasetsLinks]]
    } yield SendigDatasets(
      name = name,
      label = label,
