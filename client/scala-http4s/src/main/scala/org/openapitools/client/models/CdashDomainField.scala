...
object CdashDomainField {
  given encoderCdashDomainField: Encoder[CdashDomainField] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
        ...
        t.domainSpecific.map(v => "domainSpecific" -> v.asJson),
  ...
  given decoderCdashDomainField: Decoder[CdashDomainField] = Decoder.instance { c =>
    for {
      ...
      domainSpecific <- c.downField("domainSpecific").as[Option[String]]
      ...
      links <- c.downField("_links").as[Option[CdashDomainFieldLinks]]
    } yield CdashDomainField(
      ordinal = ordinal,
      name = name,
      label = label,
