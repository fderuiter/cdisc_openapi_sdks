...
case class SendigClasses(
    ...
    links: Option[SendigClassesLinks] = None
)
  
object SendigClasses {
  given encoderSendigClasses: Encoder[SendigClasses] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderSendigClasses: Decoder[SendigClasses] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[SendigClassesLinks]]
    } yield SendigClasses(
      name = name,
      label = label,
