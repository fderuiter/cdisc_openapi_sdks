...
case class SdtmClassVariable(
    ...
    links: Option[SdtmClassVariableLinks] = None
)
  
object SdtmClassVariable {
  given encoderSdtmClassVariable: Encoder[SdtmClassVariable] = Encoder.instance { t =>
    Json.fromFields{
      Seq(
  ...
  given decoderSdtmClassVariable: Decoder[SdtmClassVariable] = Decoder.instance { c =>
    for {
      ...
      links <- c.downField("_links").as[Option[SdtmClassVariableLinks]]
    } yield SdtmClassVariable(
      ordinal = ordinal,
      name = name,
      label = label,
