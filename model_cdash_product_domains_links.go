/*
CDISC Library API

REST API Documentation for CDISC Library in production server

API version: 1.1.0
Contact: cdisc_library_support@cdisc.org
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the CdashProductDomainsLinks type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CdashProductDomainsLinks{}

// CdashProductDomainsLinks struct for CdashProductDomainsLinks
type CdashProductDomainsLinks struct {
	Self *CdashProductDomainsRef `json:"self,omitempty"`
	ParentProduct *CdashProductRef `json:"parentProduct,omitempty"`
	Domains []CdashDomainRefElement `json:"domains,omitempty"`
}

// NewCdashProductDomainsLinks instantiates a new CdashProductDomainsLinks object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCdashProductDomainsLinks() *CdashProductDomainsLinks {
	this := CdashProductDomainsLinks{}
	return &this
}

// NewCdashProductDomainsLinksWithDefaults instantiates a new CdashProductDomainsLinks object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCdashProductDomainsLinksWithDefaults() *CdashProductDomainsLinks {
	this := CdashProductDomainsLinks{}
	return &this
}

// GetSelf returns the Self field value if set, zero value otherwise.
func (o *CdashProductDomainsLinks) GetSelf() CdashProductDomainsRef {
	if o == nil || IsNil(o.Self) {
		var ret CdashProductDomainsRef
		return ret
	}
	return *o.Self
}

// GetSelfOk returns a tuple with the Self field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CdashProductDomainsLinks) GetSelfOk() (*CdashProductDomainsRef, bool) {
	if o == nil || IsNil(o.Self) {
		return nil, false
	}
	return o.Self, true
}

// HasSelf returns a boolean if a field has been set.
func (o *CdashProductDomainsLinks) HasSelf() bool {
	if o != nil && !IsNil(o.Self) {
		return true
	}

	return false
}

// SetSelf gets a reference to the given CdashProductDomainsRef and assigns it to the Self field.
func (o *CdashProductDomainsLinks) SetSelf(v CdashProductDomainsRef) {
	o.Self = &v
}

// GetParentProduct returns the ParentProduct field value if set, zero value otherwise.
func (o *CdashProductDomainsLinks) GetParentProduct() CdashProductRef {
	if o == nil || IsNil(o.ParentProduct) {
		var ret CdashProductRef
		return ret
	}
	return *o.ParentProduct
}

// GetParentProductOk returns a tuple with the ParentProduct field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CdashProductDomainsLinks) GetParentProductOk() (*CdashProductRef, bool) {
	if o == nil || IsNil(o.ParentProduct) {
		return nil, false
	}
	return o.ParentProduct, true
}

// HasParentProduct returns a boolean if a field has been set.
func (o *CdashProductDomainsLinks) HasParentProduct() bool {
	if o != nil && !IsNil(o.ParentProduct) {
		return true
	}

	return false
}

// SetParentProduct gets a reference to the given CdashProductRef and assigns it to the ParentProduct field.
func (o *CdashProductDomainsLinks) SetParentProduct(v CdashProductRef) {
	o.ParentProduct = &v
}

// GetDomains returns the Domains field value if set, zero value otherwise.
func (o *CdashProductDomainsLinks) GetDomains() []CdashDomainRefElement {
	if o == nil || IsNil(o.Domains) {
		var ret []CdashDomainRefElement
		return ret
	}
	return o.Domains
}

// GetDomainsOk returns a tuple with the Domains field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CdashProductDomainsLinks) GetDomainsOk() ([]CdashDomainRefElement, bool) {
	if o == nil || IsNil(o.Domains) {
		return nil, false
	}
	return o.Domains, true
}

// HasDomains returns a boolean if a field has been set.
func (o *CdashProductDomainsLinks) HasDomains() bool {
	if o != nil && !IsNil(o.Domains) {
		return true
	}

	return false
}

// SetDomains gets a reference to the given []CdashDomainRefElement and assigns it to the Domains field.
func (o *CdashProductDomainsLinks) SetDomains(v []CdashDomainRefElement) {
	o.Domains = v
}

func (o CdashProductDomainsLinks) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CdashProductDomainsLinks) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Self) {
		toSerialize["self"] = o.Self
	}
	if !IsNil(o.ParentProduct) {
		toSerialize["parentProduct"] = o.ParentProduct
	}
	if !IsNil(o.Domains) {
		toSerialize["domains"] = o.Domains
	}
	return toSerialize, nil
}

type NullableCdashProductDomainsLinks struct {
	value *CdashProductDomainsLinks
	isSet bool
}

func (v NullableCdashProductDomainsLinks) Get() *CdashProductDomainsLinks {
	return v.value
}

func (v *NullableCdashProductDomainsLinks) Set(val *CdashProductDomainsLinks) {
	v.value = val
	v.isSet = true
}

func (v NullableCdashProductDomainsLinks) IsSet() bool {
	return v.isSet
}

func (v *NullableCdashProductDomainsLinks) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCdashProductDomainsLinks(val *CdashProductDomainsLinks) *NullableCdashProductDomainsLinks {
	return &NullableCdashProductDomainsLinks{value: val, isSet: true}
}

func (v NullableCdashProductDomainsLinks) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCdashProductDomainsLinks) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


