#' Create a new ExportSdtmDatasetsRow
#'
#' @description
#' ExportSdtmDatasetsRow Class
#'
#' @docType class
#' @title ExportSdtmDatasetsRow
#' @description ExportSdtmDatasetsRow Class
...
#' @export
ExportSdtmDatasetsRow <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ExportSdtmDatasetsRow class.
    #'
    ...
    #'
    #' @return ExportSdtmDatasetsRow as a base R list.
    #' @examples
    #' # convert array of ExportSdtmDatasetsRow (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ExportSdtmDatasetsRow to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ExportSdtmDatasetsRowObject <- list()
      if (!is.null(self$`Version`)) {
        ExportSdtmDatasetsRowObject[["Version"]] <-
          self$`Version`
      }
      if (!is.null(self$`Class`)) {
        ExportSdtmDatasetsRowObject[["Class"]] <-
          self$`Class`
      }
      if (!is.null(self$`Dataset Name`)) {
        ExportSdtmDatasetsRowObject[["Dataset Name"]] <-
          self$`Dataset Name`
      }
      if (!is.null(self$`Dataset Label`)) {
        ExportSdtmDatasetsRowObject[["Dataset Label"]] <-
          self$`Dataset Label`
      }
      if (!is.null(self$`Structure`)) {
        ExportSdtmDatasetsRowObject[["Structure"]] <-
          self$`Structure`
      }
      return(ExportSdtmDatasetsRowObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ExportSdtmDatasetsRow
    #'
    #' @param input_json the JSON input
    #' @return the instance of ExportSdtmDatasetsRow
    ...
    #' @return ExportSdtmDatasetsRow in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ExportSdtmDatasetsRow and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ExportSdtmDatasetsRow
    toString = function() {
      self$toJSONString()
    },
...
# ExportSdtmDatasetsRow$unlock()
#
## Below is an example to define the print function
# ExportSdtmDatasetsRow$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ExportSdtmDatasetsRow$lock()
